...
async function startServer() {
  ...
  app.get("/api/public/data", async (req, res) => {
    try {
      ...
      const [aboutRows] = await pool.execute("SELECT * FROM about LIMIT 1");
  ...
  app.get("/api/admin/dashboard-stats", authenticate, async (req, res) => {
    try {
      ...
      const [totalFleet] = await pool.execute("SELECT COUNT(*) as count FROM fleet");
      const [totalArticles] = await pool.execute("SELECT COUNT(*) as count FROM articles");
